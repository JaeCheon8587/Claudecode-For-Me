...
- `--process <process-dir>`가 있으면 해당 `handoff.json`을 단일 ssot-write 입력으로 사용한다.
- 같은 process의 `build.md`, `progress.md`, `handoff.json`이 없거나 progress/handoff 상태가 `SUCCESS`가 아니면 실행하지 않는다.
...
- `handoff.json.actions`의 `source_paths`, `target_path`, `authority_paths`, `instruction`, `acceptance_criteria`, `modifications`를 Required 입력과 실행 규칙에 반영한다. 명시적 authority 없는 Action은 `Draft`로 둔다.
...
- auditor input에는 `handoff.json actions`, `Authority Inputs and Instructions`, `Expected Required SSOT Execution Matrix`, `Impact / source summary`를 채워 전달한다.
