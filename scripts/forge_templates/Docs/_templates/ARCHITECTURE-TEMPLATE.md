...
호스트 프로젝트 경로는 `{호스트 루트 패턴, 예: Src/App/{SOLUTION_NAME}.{Host}/}` 패턴이다. Host ∈ {`{호스트 1}`, `{호스트 2}`, ...}.

### 3.1 {호스트 1 이름} — {호스트 1 한 줄 설명}

{TFM 또는 런타임, 예: net6.0 / Node 18 / JVM 17}.

- **반드시** {호스트 1의 핵심 책임 1}.
- **반드시** {핵심 책임 2}.
- {도메인 규칙·외부 IO 구현체·{호스트 1과 무관한 기능}}을 {호스트 1} 프로젝트에 두는 것은 **금지**.
- 호스트는 **조립(DI 등록·생명주기)만** 수행한다. {외부 IO Adapter 구현체} 위치 정책 명기.

### 3.2 {호스트 2 이름} — {호스트 2 한 줄 설명}

- **반드시** {호스트 2의 핵심 책임 1}.
- **반드시** {핵심 책임 2}.
- {호스트 2가 절대 참조하면 안 되는 모듈} 참조는 **절대 금지**.

> 호스트가 {N}개 이상이면 §3.{N}을 추가한다.
...
> 도메인이 특정 Cross-Cutting 모듈(예: UI 추상화·통신 추상화)에 의존하지 않아야 한다면 본 절에 예외를 명시한다.
