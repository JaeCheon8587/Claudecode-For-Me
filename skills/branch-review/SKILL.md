...
argument-hint: "[고정점 ref — 미지정 시 merge-base 자동 추정] [--spec <path> ...] [--resume]"
---
...
1. `$ARGUMENTS` 파싱: `ref`(위치 인자, 선택 — git ref 전용) + `--spec <path>`(선택, 여러 번 가능) + `--resume`(플래그, 선택).
...
3. `.process/branch-review-<slug>/` 존재 확인:
   ...
   - **있음 + `--resume`**: `branch-review-progress.md`를 Read. `Stage Status`에서 `done`이 아닌 첫 항목부터 재개한다. `Chunk Status` 표가 있으면 `done`인 청크는 `.process/branch-review-<slug>/chunk-<id>.log`에 저장된 4 finder raw 출력을 그대로 재사용(재실행하지 않음), `pending`/`doing`/`blocked`인 청크만 Step 4에서 재실행 대상으로 표시한다. `progress.md`의 `Log`는 raw 원문이 아니라 해당 log 파일 경로를 가리키는 인덱스다. `branch-review-build.md`의 기존 Inputs/Routing 값(Spec/Standards 등급 등)도 재사용해 Step 1~3 재계산을 생략할 수 있다(단, HEAD sha가 slug와 일치하는지 재확인 — 불일치하면 "재개 대상 없음, HEAD 이동됨 — 새 리뷰로 시작" 안내 후 신규 실행).
...
**입력 우선순위**:
1. 사용자가 ref 명시 (`--spec`으로 받은 파일 경로는 ref로 해석하지 않음)
...
- 빌드 산출물 디렉토리: top-level `dist`, `build`, `out`, `node_modules` (예: `build/app.js` 제외, `src/build/helper.py` 유지)
...
2. 그룹이 캡(1500줄/30파일) 초과 시 그룹 내부에서 파일 단위 greedy bin-packing으로 서브청크 분할(경로 알파벳순, 결정적 재현 가능). 단일 파일 하나가 1500줄을 넘으면 파일 단위 분할 한계상 해당 청크는 라인 cap을 초과할 수 있으며, 스크립트 출력 `Warnings`에 이를 남긴다.
...
2. **사용자 명시 경로** — `--spec <path>`로 받은 spec 파일(여러 번 가능). 위치 인자는 ref 전용이므로 bare file path를 spec으로 추측하지 않는다.
...
NIT 항목은 기본 모드에서 finder 자체가 Step 4 공통 출력 제약("카운트 + 대표 2건")으로 축약해 verbatim 출력에 포함한다 — 6-1 verbatim 원칙에 따라 그 대표 2건은 최종 리포트에도 그대로 노출된다(이는 정상이며 본 조항 위반이 아니다). 사용자가 리뷰 시작 시 "NIT 포함" 또는 verbose를 요청하면 Step 4 프롬프트 치환 시 4개 finder 모두의 MINOR/NIT 제한을 "전부 보고"로 완화한다. 이미 기본 모드로 완료된 `chunk-<id>.log`에는 대표 2건만 있을 수 있으므로, 사후 verbose 전환은 기존 raw log에서 보존된 범위까지만 펼친다(없는 NIT를 새로 재구성하지 않음).
...
- **생성 파일 배제는 확장자 일반화가 아님** — lockfile/top-level 빌드산출물/이미지·폰트·`.min.*`/`linguist-generated` 속성만 자동 제외한다. 이름만으로 "생성됨"이 짐작되는 개별 파일(예: 리포트 스냅샷 `.html`)은 자동 제외 대상이 아니다 — `.gitattributes`에 `linguist-generated` 명시가 정석 경로.
