...
- **exit code 2** (검증 게이트 미통과): stderr에 `§F` 메시지로 미결 항목(원시 템플릿 / §11 미확인 사항 Open / §7 결정 필요 / 미치환 placeholder)이 나온다. 미결 항목을 사용자에게 **그대로 전달하고 중단**한다 — 문서를 먼저 완성/확정해야 한다. **세션은 TASK 문서나 어떤 입력 문서도 게이트 통과 목적으로 직접 수정하지 않는다.** 입력 문서는 SSOT이며 gitignored일 수 있어, 자동 수정은 흔적 없이 원본을 오염시킨다. 문서 정정은 사용자 몫이다.
