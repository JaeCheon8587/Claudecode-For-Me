...
| ssot-write | TASK path, Opus main session | optional `--app <APP>`, optional `--name <slug>`, optional `--resume` | SSOT CREATE/UPDATE, `.process/<TASK-stem>/ssot-write-{build,progress,impact,action,audit}.md` | Opus consistency audit `PASS`; required subagent unavailable/`BLOCKED`/repair cap이면 중단 | TASK path + ssot process dir |
...
- `ssot-write-build.md`의 `Confirmed SSOT Action Matrix`는 Sonnet actor의 수정 권한이자 Opus audit 및 `work-packet-write`의 공유 입력이다. pipeline-runner가 matrix 본문을 메인 컨텍스트로 복사하지 않는다.
