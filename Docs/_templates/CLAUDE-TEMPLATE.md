...
- **SYSTEM_CODE** ≡ **APP_CODE** ≡ **{App}**: App(S/W 단위) 식별자. 동일 개념의 3개 별칭. 본 파일의 § Backend Services Overview 표가 단일 출처(SSOT). 예: `LOADER`. App별 문서 ID 패턴 `{SYSTEM_CODE}-PRD`, `{SYSTEM_CODE}-FC`, `{SYSTEM_CODE}-FRD-{NNN}`, `{SYSTEM_CODE}-TASK-{NNN}`, `{SYSTEM_CODE}-ARD-{NNN}` 등에 사용. (RFD 양식은 DOCUMENT_GUIDE v0.7 폐기 — 리팩토링은 TASK 작업 유형 = `refactor`.)
...
| **App: {SYSTEM_CODE}** | [`Docs/{SYSTEM_CODE}/`](Docs/{SYSTEM_CODE}/) | App별 PRD/FC/ARCHITECTURE/FRD/TASK/ARD/ARD-CATALOG SSOT 폴더 |
...
| `_templates/App/APP-ARD-CATALOG-TEMPLATE.md` | App ARD Catalog | `Docs/{SYSTEM_CODE}/{SYSTEM_CODE}-ARD-CATALOG.md` |
| `_templates/App/ARD/APP-ARD-001-TEMPLATE.md` | App ARD 본문 (단일 ARD 1건 base) | `Docs/{SYSTEM_CODE}/ARD/{SYSTEM_CODE}-ARD-{NNN}.md` |
...
Redirect/Legacy 양식 (`ARD-TEMPLATE.md`, `FC-TEMPLATE.md`, `FRD-TEMPLATE.md`, `README-TEMPLATE.md`, `UI_GUIDE-TEMPLATE.md`) 은 0.2 이전 전역 문서 구조 이식 확인용이다. 신규 문서 작성에는 위 Active 양식만 사용한다.

ARD 명칭은 [`Docs/DOCUMENT_GUIDE.md`](Docs/DOCUMENT_GUIDE.md) 0.3 기준이다. 부트스트랩/신규 작성 시 템플릿 파일명, 결과 파일명, 문서 ID 모두 `ARD` 를 사용한다.

## Backend Services Overview

본 솔루션의 App 레지스트리. **SYSTEM_CODE 단일 출처(SSOT)**. 신규 App 도입 시 본 표 행 추가가 모든 다른 작업(PRD/FC/FRD/TASK/ARD 작성) 보다 선행. App 다수 시 행 복제.
...
- **신규 기능 작성 흐름** ([DOCUMENT_GUIDE §2](Docs/DOCUMENT_GUIDE.md#2-작성-순서) 준수):
  ...
  4. 필요 시 `Docs/{SYSTEM_CODE}/ARD/{SYSTEM_CODE}-ARD-{NNN}.md` 등재 (`_templates/App/ARD/APP-ARD-001-TEMPLATE.md` 복사) + `{SYSTEM_CODE}-ARD-CATALOG.md` 동기화
...
- **AI 실행용 작업 지시서 (TASK) 작성 흐름** — 모든 코드 작업 (feature / refactor / maintenance / migration / setup / investigation) 통합:
  1. (사전) 영향 영구 SSOT (PRD/FC/FRD/ARD/ARD-CATALOG/ARCHITECTURE) 를 작성자가 직접 갱신
...
- **신규 App 추가**:
  ...
  3. `Docs/{SYSTEM_CODE}/` 폴더 생성 + 하위 `FRD/`, `ARD/`, `TASK/` 서브폴더 생성 (RFD 폐기 — v0.7)
  4. `_templates/App/` 의 4 종 직접 양식 복사·rename:
     ...
     - `APP-ARD-CATALOG-TEMPLATE.md` → `Docs/{SYSTEM_CODE}/{SYSTEM_CODE}-ARD-CATALOG.md`
  5. 첫 ARD/FRD/TASK 작성 시 `_templates/App/ARD/`, `_templates/App/FRD/`, `_templates/App/TASK/` 서브폴더 양식 사용 (RFD 폐기).
  6. 솔루션 공통 양식 (`_templates/PRD-TEMPLATE.md`, `_templates/ARCHITECTURE-TEMPLATE.md`) 은 신규 App 시 사용 안 함.
- **신규 ARD 등재**: `_templates/App/ARD/APP-ARD-001-TEMPLATE.md` 복사 → `Docs/{SYSTEM_CODE}/ARD/{SYSTEM_CODE}-ARD-{NNN}.md` → `{SYSTEM_CODE}-ARD-CATALOG.md` Proposed/Accepted 행 추가 → 영향 PRD/FC/FRD 본문에 ARD 인용 (TASK 인용 X — v0.7 룰).
...
6. `Docs/_templates/` 폴더는 **원본 보존** — 추후 신규 App/ARD/FRD/TASK/룰 추가 시 재참조.
...
- `Docs/_templates/**` — 원본 양식. 사용자 승인 전 수정 금지 (신규 App/ARD/FRD/TASK/룰 부트스트랩 시 재참조).
